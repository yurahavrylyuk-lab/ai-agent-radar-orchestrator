...
Revision 3 corrects the remaining evidence-retention defect. Revision 2 redirected the complete TAP stream to a temporary file and did not emit it until the runner exited; interruption triggered cleanup before replay, producing `cat: ... No such file` and leaving observers with only the preamble. Revision 3 mirrors TAP live through a FIFO and `tee` while retaining the same bytes for strict post-run parsing. It preserves the runner status independently, rejects missing, duplicate, malformed, or arithmetically inconsistent TAP summaries, and reports wrapper failures separately. Two consecutive exact public-wrapper runs each emitted `OFFLINE_RUNNER_AGGREGATE tests=154 pass=154 fail=0` and `OFFLINE_WRAPPER_CHECKS pass=12 fail=0`; all values came from the current run or executed wrapper checks, with no historical total embedded in the wrapper.
