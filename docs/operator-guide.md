...
Boundary policy paths are derived from verified filesystem identities, not lexical spelling. On macOS, Node may report the temporary root below `/var` while the native sandbox evaluates its canonical `/private/var` path. The controller accepts that one known alias only after proving both spellings resolve to the same directory device and inode; arbitrary symlink leaves or ancestor redirections remain rejected. It captures directory type, device, inode, owner, and mode, rejects traversal and protected/output overlap, uses the same canonical roots in the sandbox profile and probe environment, and rechecks identities before and after each probe. Every direct and descendant file check and both network checks must report independently; a missing result, unexpected error, permitted forbidden operation, or failed output control blocks activation.

This correction does not resume the stopped authorization. Any eventual pilot must bind to the independently reviewed, accepted, and published release containing the correction. The existing request and approval must not be rewritten automatically if their release binding no longer matches; that conflict requires an explicit human decision.

The trusted operator can run the standalone real-path diagnostic independently before any later authorization attempt. Create an empty mode-`0700` output directory under `/private/tmp`, call `runRealConfinementDiagnostic({ roleOutputRoot })` from `src/operator-boundary.mjs`, retain the returned JSON evidence, and remove only that temporary output directory afterward. The diagnostic fixes authority, controller, and protected-target roots in controller policy; only the designated output root is supplied by the operator. It uses native `sandbox_check` queries and does not attempt protected filesystem mutations. It validates direct and descendant authority read/write, controller write, target write, output write, and inbound/outbound network policy, with filesystem identity checks before and after. It does not call `authorize-pilot` or create a proposal, digest, ledger, grant, cycle, or integration intent.

`scripts/test-offline.sh` captures the complete TAP stream, re-emits it, extracts the current runner's own `tests`, `pass`, and `fail` lines, and prints them as `OFFLINE_RUNNER_AGGREGATE`. It separately prints `OFFLINE_WRAPPER_CHECKS`; these are shell-boundary checks, not Node test cases. Neither total is hardcoded.
