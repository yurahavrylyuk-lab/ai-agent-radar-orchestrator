...
import { controllerStatus, enqueueRequest, initializeController, newestSummary, runRehearsal } from "./coordinator.mjs";
...
async function main(argv) {
  ...
  if (command === "next-task") { if (state.pendingTaskId === null) { print({ status: "NO_PENDING_TASK" }); return; } const task = state.tasks.find((item) => item.taskId === state.pendingTaskId); print(renderTaskPrompt(task)); return; }
  ...
  if (command === "submit-result") { const file = option(argv, "--file"); if (!file) throw new Error("RESULT_FILE_REQUIRED"); print(submitRoleResult({ statePath, ownerId, ownerGeneration, result: readJson(file), now: new Date().toISOString() }).value); return; }
  if (command === "start-role") { const taskId = option(argv, "--task"); if (!taskId) throw new Error("TASK_ID_REQUIRED"); print(startRole({ statePath, ownerId, ownerGeneration, taskId, now: new Date().toISOString() }).value); return; }
  if (command === "finish-role") { const taskId = option(argv, "--task"); if (!taskId) throw new Error("TASK_ID_REQUIRED"); print(finishRole({ statePath, ownerId, ownerGeneration, taskId, now: new Date().toISOString() }).value); return; }
  if (command === "checkpoint") { const taskId = option(argv, "--task"); if (!taskId) throw new Error("TASK_ID_REQUIRED"); print(createLocalCheckpoint({ statePath, ownerId, ownerGeneration, taskId, now: new Date().toISOString() })); return; }
