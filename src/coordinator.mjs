...
export async function runRehearsal(runtimeRoot, options = {}) {
  ...
  const { attestedValidationEntries } = await import("./validation-evidence.mjs");
  ...
  async function builderRound(content) {
    ...
    const checkpoint = createLocalCheckpoint({ statePath, ownerId, ownerGeneration: 1, taskId: builderTask.taskId, now: now() }).receipt; const validation = attestedValidationEntries(readState(statePath), builderTask, "PASS");
  ...
  executeRole(task); const validation1 = attestedValidationEntries(readState(statePath), task, "FAIL"); submitRoleResult({ statePath, ownerId, ownerGeneration: 1, result: resultFor(task, { reviewedCommit: candidate1, reviewState: "REVISE", findings: ["The required record-comparison validation failed because distinguishing fields were incomplete."], requiredChanges: ["Add a record comparison example."], recommendations: [], validation: validation1 }), now: now() });
  ...
  executeRole(task); submitRoleResult({ statePath, ownerId, ownerGeneration: 1, result: resultFor(task, { reviewedCommit: candidate2, reviewState: "PASS", findings: [], requiredChanges: [], recommendations: [], validation: attestedValidationEntries(readState(statePath), task, "PASS") }), now: now() });
