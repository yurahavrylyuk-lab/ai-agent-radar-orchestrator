...
function validateEvidence(value, name = "previousEvidence") {
  ...
  value.records.forEach((record, index) => {
    exactRecord(record, ["type", "id", "digest"], `${name}.records[${index}]`);
...
function validateLimits(value, name = "limits") {
  const fields = ["maxIterations", "maxExecutionSeconds", "maxAdditionalCostUsd", "network", "providers", "publication", "scheduling"];
  exactRecord(value, fields, name);
  if (value.maxIterations !== PHASE2.maxIterations || value.maxExecutionSeconds !== PHASE2.maxExecutionSeconds || value.maxAdditionalCostUsd !== 0) throw new TypeError(`${name} does not match fixed Phase 2 limits`);
...
function validateValidationEntry(value, name) {
  exactRecord(value, ["recipeId", "outcome", "evidenceDigest", "skipReason"], name); string(value.recipeId, `${name}.recipeId`); oneOf(value.outcome, ["PASS", "FAIL", "SKIPPED"], `${name}.outcome`); digest(value.evidenceDigest, `${name}.evidenceDigest`, true); nullableString(value.skipReason, `${name}.skipReason`);
  if (value.outcome === "SKIPPED" && value.skipReason === null) throw new TypeError(`${name}.skipReason is required for SKIPPED`);
  if (value.outcome !== "SKIPPED" && value.evidenceDigest === null) throw new TypeError(`${name}.evidenceDigest is required`);
}
...
export function validateRoleResultV2(value, task = null) {
  ...
  if (task) {
    ...
    if (value.outcome === "COMPLETED" && Array.isArray(value.payload.validation)) {
      const allowed = new Set(task.authorization.validationRequirements.map((item) => item.id));
      if (value.payload.validation.some((item) => !allowed.has(item.recipeId))) throw new TypeError("result references an unapproved validation recipe");
    }
  }
  return true;
}
...
export function validateMachineStateV2(value) {
  ...
  value.results.forEach((record, index) => { const n = `results[${index}]`; exactRecord(record, ["taskId", "taskDigest", "cycleId", "role", "purpose", "resultDigest", "result", "acceptedAt"], n); const task = value.tasks.find((item) => item.taskId === record.taskId); if (!task) throw new TypeError(`${n} references a missing task`); digest(record.resultDigest, `${n}.resultDigest`); if (sha256Canonical(record.result) !== record.resultDigest) throw new TypeError(`${n}.resultDigest mismatch`); validateRoleResultV2(record.result, task); utcTimestamp(record.acceptedAt, `${n}.acceptedAt`); });
  ...
  value.timings.forEach((item, index) => { const n = `timings[${index}]`; exactRecord(item, ["taskId", "status", "waitingStartedAt", "startedAt", "finishedAt", "activeMs", "waitingMs"], n); string(item.taskId, `${n}.taskId`); oneOf(item.status, ["WAITING", "EXECUTING"], `${n}.status`); utcTimestamp(item.waitingStartedAt, `${n}.waitingStartedAt`); utcTimestamp(item.startedAt, `${n}.startedAt`); if (item.finishedAt !== null) utcTimestamp(item.finishedAt, `${n}.finishedAt`); integer(item.activeMs, `${n}.activeMs`); integer(item.waitingMs, `${n}.waitingMs`); });
