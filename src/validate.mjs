...
function validateCheckoutEvidence(value, name) {
  const fields = ["head", "symbolicHead", "semanticIndex", "manifestDigest", "modesDigest", "configDigest", "gitDirectoryIdentity", "statusDigest", "statusPorcelainV1Base64", "fileCount", "pilotPathAbsent"];
  exactRecord(value, fields, name); oid(value.head, `${name}.head`); string(value.symbolicHead, `${name}.symbolicHead`);
  exactRecord(value.semanticIndex, ["format", "digest", "entryCount", "equalsHeadTree", "ordinaryFlagsOnly", "cachedDiffEmpty"], `${name}.semanticIndex`); if (value.semanticIndex.format !== "git-index-semantic-v1") throw new TypeError(`${name}.semanticIndex.format is incompatible`); digest(value.semanticIndex.digest, `${name}.semanticIndex.digest`); integer(value.semanticIndex.entryCount, `${name}.semanticIndex.entryCount`); for (const field of ["equalsHeadTree", "ordinaryFlagsOnly", "cachedDiffEmpty"]) bool(value.semanticIndex[field], `${name}.semanticIndex.${field}`);
  for (const field of ["manifestDigest", "modesDigest", "configDigest", "statusDigest"]) digest(value[field], `${name}.${field}`); integer(value.fileCount, `${name}.fileCount`, 1); if (value.pilotPathAbsent !== true) throw new TypeError(`${name}.pilotPathAbsent must be true`);
  exactRecord(value.gitDirectoryIdentity, ["canonicalPath", "device", "inode"], `${name}.gitDirectoryIdentity`); string(value.gitDirectoryIdentity.canonicalPath, `${name}.gitDirectoryIdentity.canonicalPath`); integer(value.gitDirectoryIdentity.device, `${name}.gitDirectoryIdentity.device`); integer(value.gitDirectoryIdentity.inode, `${name}.gitDirectoryIdentity.inode`);
  if (typeof value.statusPorcelainV1Base64 !== "string" || Buffer.from(value.statusPorcelainV1Base64, "base64").toString("base64") !== value.statusPorcelainV1Base64) throw new TypeError(`${name}.statusPorcelainV1Base64 must be canonical Base64`);
  return true;
}
export function validateCheckoutCondition(value, name = "checkoutCondition") {
  exactRecord(value, ["schemaVersion", "status", "synchronizedToNewHead", "destructiveCheckoutPerformed", "synchronizationRequirement", "preIntegration", "integratedBranch", "postIntegration"], name);
  if (value.schemaVersion !== 1 || value.status !== "REF_ADVANCED_CHECKOUT_PRESERVED" || value.synchronizedToNewHead !== false || value.destructiveCheckoutPerformed !== false || value.synchronizationRequirement !== "SEPARATE_HUMAN_CONTROLLED_ACTION_REQUIRED") throw new TypeError(`${name} fixed policy mismatch`);
  validateCheckoutEvidence(value.preIntegration, `${name}.preIntegration`); validateCheckoutEvidence(value.postIntegration, `${name}.postIntegration`);
  const branchFields = ["targetBranch", "symbolicHead", "updatedRef", "expectedOldTip", "newTip", "candidateTree", "scopeDigest", "reviewResultDigest", "architectResultDigest"];
  exactRecord(value.integratedBranch, branchFields, `${name}.integratedBranch`); for (const field of ["targetBranch", "symbolicHead", "updatedRef"]) string(value.integratedBranch[field], `${name}.integratedBranch.${field}`); for (const field of ["expectedOldTip", "newTip", "candidateTree"]) oid(value.integratedBranch[field], `${name}.integratedBranch.${field}`); for (const field of ["scopeDigest", "reviewResultDigest", "architectResultDigest"]) digest(value.integratedBranch[field], `${name}.integratedBranch.${field}`);
  const expectedRef = `refs/heads/${PHASE2.pilotBranch}`; if (value.integratedBranch.targetBranch !== PHASE2.pilotBranch || value.integratedBranch.symbolicHead !== expectedRef || value.integratedBranch.updatedRef !== expectedRef) throw new TypeError(`${name}.integratedBranch identity mismatch`);
  if (value.preIntegration.head !== value.integratedBranch.expectedOldTip || value.postIntegration.head !== value.integratedBranch.newTip || value.preIntegration.symbolicHead !== expectedRef || value.postIntegration.symbolicHead !== expectedRef) throw new TypeError(`${name} head relationship mismatch`);
  if (!value.preIntegration.semanticIndex.equalsHeadTree || !value.preIntegration.semanticIndex.ordinaryFlagsOnly || !value.preIntegration.semanticIndex.cachedDiffEmpty || value.preIntegration.statusPorcelainV1Base64 !== "" || value.preIntegration.statusDigest !== sha256Bytes(Buffer.alloc(0))) throw new TypeError(`${name}.preIntegration is not a clean authorized baseline`);
  if (value.postIntegration.semanticIndex.equalsHeadTree || !value.postIntegration.semanticIndex.ordinaryFlagsOnly || value.postIntegration.semanticIndex.cachedDiffEmpty) throw new TypeError(`${name}.postIntegration is not the expected unsynchronized checkout`);
  for (const field of ["manifestDigest", "modesDigest", "configDigest", "fileCount"]) if (value.postIntegration[field] !== value.preIntegration[field]) throw new TypeError(`${name}.${field} was not preserved`);
  if (value.postIntegration.semanticIndex.format !== value.preIntegration.semanticIndex.format || value.postIntegration.semanticIndex.digest !== value.preIntegration.semanticIndex.digest || value.postIntegration.semanticIndex.entryCount !== value.preIntegration.semanticIndex.entryCount || JSON.stringify(value.postIntegration.gitDirectoryIdentity) !== JSON.stringify(value.preIntegration.gitDirectoryIdentity)) throw new TypeError(`${name} index or repository identity was not preserved`);
  const expectedStatus = Buffer.from(`D  ${PHASE2.pilotPath}\0`, "utf8"); if (value.postIntegration.statusPorcelainV1Base64 !== expectedStatus.toString("base64") || value.postIntegration.statusDigest !== sha256Bytes(expectedStatus)) throw new TypeError(`${name}.postIntegration status mismatch`);
  return true;
}
export function validateIntegrationIntent(value) {
  ...
  const fields = ["schemaVersion", "intentId", ...authorizationFields, "repositoryId", "cycleId", "targetRoot", "targetBranch", "oldTip", "newTip", "candidateTree", "scopeDigest", "reviewResultDigest", "architectResultDigest", ...(value.schemaVersion === 4 ? ["checkoutBaseline"] : []), "status", "createdAt"];
  exactRecord(value, fields, "integrationIntent"); if (![2, 3, 4].includes(value.schemaVersion)) throw new TypeError("integrationIntent.schemaVersion must be 2, 3, or 4");
  ...
  if (value.schemaVersion === 4) { validateCheckoutEvidence(value.checkoutBaseline, "integrationIntent.checkoutBaseline"); if (!value.checkoutBaseline.semanticIndex.equalsHeadTree || !value.checkoutBaseline.semanticIndex.cachedDiffEmpty || !value.checkoutBaseline.semanticIndex.ordinaryFlagsOnly || value.checkoutBaseline.statusPorcelainV1Base64 !== "" || value.checkoutBaseline.head !== value.oldTip) throw new TypeError("integrationIntent.checkoutBaseline is invalid"); }
...
export function validatePilotAuthorizationGrant(value) {
  ...
  exactRecord(value, fields, "pilotAuthorization"); if (value.schemaVersion !== 2) throw new TypeError("pilotAuthorization.schemaVersion must be 2");
  ...
  exactRecord(value.targetSnapshot, ["manifestDigest", "modesDigest", "configDigest", "rawIndexDigestDiagnostic", "semanticIndex", "fileCount", "gitDirectoryIdentity", "protectedRefs"], "pilotAuthorization.targetSnapshot");
  for (const field of ["manifestDigest", "modesDigest", "configDigest", "rawIndexDigestDiagnostic"]) digest(value.targetSnapshot[field], `pilotAuthorization.targetSnapshot.${field}`); integer(value.targetSnapshot.fileCount, "pilotAuthorization.targetSnapshot.fileCount", 1); validateProtectedRefs(value.targetSnapshot.protectedRefs, "pilotAuthorization.targetSnapshot.protectedRefs");
  exactRecord(value.targetSnapshot.semanticIndex, ["format", "digest", "entryCount", "equalsHeadTree", "ordinaryFlagsOnly", "cachedDiffEmpty"], "pilotAuthorization.targetSnapshot.semanticIndex"); if (value.targetSnapshot.semanticIndex.format !== "git-index-semantic-v1") throw new TypeError("pilotAuthorization semantic index format is incompatible"); digest(value.targetSnapshot.semanticIndex.digest, "pilotAuthorization.targetSnapshot.semanticIndex.digest"); integer(value.targetSnapshot.semanticIndex.entryCount, "pilotAuthorization.targetSnapshot.semanticIndex.entryCount"); for (const field of ["equalsHeadTree", "ordinaryFlagsOnly", "cachedDiffEmpty"]) if (value.targetSnapshot.semanticIndex[field] !== true) throw new TypeError(`pilotAuthorization.targetSnapshot.semanticIndex.${field} must be true`);
...
export function validateMachineStateV2(value) {
  ...
  value.integrationOutcomes.forEach((item, index) => { const n = `integrationOutcomes[${index}]`; if (item.status === "INTEGRATED") { const fields = ["intentId", "cycleId", "status", "oldTip", "newTip", ...(item.checkoutCondition === undefined ? [] : ["checkoutCondition"]), "verifiedAt"]; exactRecord(item, fields, n); oid(item.oldTip, `${n}.oldTip`); oid(item.newTip, `${n}.newTip`); if (item.checkoutCondition !== undefined) validateCheckoutCondition(item.checkoutCondition, `${n}.checkoutCondition`); utcTimestamp(item.verifiedAt, `${n}.verifiedAt`); } else { exactRecord(item, ["intentId", "cycleId", "status", "observedTip", "observedAt"], n); if (item.status !== "INTEGRATION_RECONCILIATION_REQUIRED") throw new TypeError(`${n}.status is invalid`); oid(item.observedTip, `${n}.observedTip`); utcTimestamp(item.observedAt, `${n}.observedAt`); } string(item.intentId, `${n}.intentId`); string(item.cycleId, `${n}.cycleId`); });
...
export function validateAuthorityState(value, statePath = null) {
  ...
  base.integrationOutcomes = base.integrationOutcomes.map((item) => item.status === "INTEGRATED" && item.checkoutCondition !== undefined ? { intentId: item.intentId, cycleId: item.cycleId, status: item.status, oldTip: item.oldTip, newTip: item.newTip, verifiedAt: item.verifiedAt } : item);
  const activationSummaryFields = ["authorizationId", "authorizationDigest", "controllerCommit", "requestId", "baselineCommit", "protectedTargetSnapshot", "candidateCommits", "analystReviews", "validationAttestations", "architectDecisions", "integrationIntent", "changedPath", "protectedTargetComparison", "checkoutCondition", "limitations"];
  ...
  if (value.integrationIntents.some((item) => ![3, 4].includes(item.schemaVersion))) throw new TypeError("authority state integration intents must use schemaVersion 3 or 4");
  ...
  value.summaries.filter((item) => item.type === "FINAL" && item.evidenceMode === "HUMAN_ASSISTED").forEach((item, index) => {
    ...
    if (item.checkoutCondition !== null) validateCheckoutCondition(item.checkoutCondition, `${n}.checkoutCondition`); if (item.terminalReason === "INTEGRATED" && item.checkoutCondition === null) throw new TypeError(`${n}.checkoutCondition is required for integration`);
