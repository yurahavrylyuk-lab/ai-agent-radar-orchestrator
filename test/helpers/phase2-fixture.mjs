...
export const approval = Object.freeze({ approvalId: "gov-002-test", scope: "Add the offline guide only.", allowedChanges: [{ path: "docs/learning/offline-fixture-reading.md", operation: "ADD" }], forbiddenChanges: ["No network", "No providers"], acceptanceCriteria: [{ id: "criterion", description: "The offline guide is bounded." }], validationRequirements: [{ id: "recipe", description: "Validate the bounded guide." }] });
...
export function analystState(root, { iteration = 1, reviewCommit = OID_B } = {}) {
  ...
  state.cycles.push(cycle); state.activeCycleId = cycle.id; state.plans.push({ cycleId: cycle.id, ...plan, architectResultDigest: DIGEST, createdAt: cycle.createdAt }); state.iterations.push({ id: `iteration:${iteration}`, cycleId: cycle.id, index: iteration, planRevision: iteration, candidateCommit: reviewCommit, checkpointReceiptId: `checkpoint:${iteration}`, reviewResultDigest: null });
  ...
  state.tasks.push(task); state.pendingTaskId = task.taskId; return { state, task };
}
export function analystPayload(commit, reviewState) { return { reviewedCommit: commit, reviewState, findings: reviewState === "PASS" ? [] : ["finding"], requiredChanges: reviewState === "REVISE" ? ["change"] : [], recommendations: reviewState === "PASS_WITH_RECOMMENDATIONS" ? ["recommendation"] : [], validation: [{ recipeId: "recipe", outcome: "PASS", evidenceDigest: sha256Canonical({ reviewed: commit }), skipReason: null }] }; }
