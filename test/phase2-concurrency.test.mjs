...
test("two real processes competing for one task produce one accepted transaction", async () => {
  const root = runtimeDirectory("concurrency"); const statePath = path.join(root, "state.json"); const resultPath = path.join(root, "result.json"); const { state, task } = analystState(root); initializeStateV2(statePath, state); fs.writeFileSync(resultPath, `${JSON.stringify(makeResult(task, analystPayload(OID_B, "PASS")), null, 2)}\n`);
