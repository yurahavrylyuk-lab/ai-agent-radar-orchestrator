...
test("checkpoint interruption after commit creates a durable reconciliation hold and prohibits retry", () => {
  ...
  let state = readState(statePath); let task = state.tasks.at(-1); const payload = { goal: "guide", scope: approval.scope, allowedChanges: approval.allowedChanges, forbiddenChanges: approval.forbiddenChanges, acceptanceCriteria: approval.acceptanceCriteria, validationRequirements: approval.validationRequirements, risks: [], rationale: "bounded", resolvesFindingIds: [] }; submitRoleResult({ statePath, ownerId: "owner", ownerGeneration: 1, result: makeResult(task, payload), now: "2026-09-21T00:00:01.000Z" });
